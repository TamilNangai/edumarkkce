...
export default function Reports() {
  ...
  filteredMarks.forEach((m: any) => {
    if (m.marks_obtained === -1) return; // skip absent
  ...
  // Consolidated report: all subjects for each student, even if marks missing
    ...
    return classStudents.map((student: any) => {
      const subjectMarks = subjects.map((sub: any) => {
        ...
        return {
          ...
          obtained: mark ? mark.marks_obtained : null,
          max: mark ? mark.max_marks : null,
          absent: mark?.marks_obtained === -1,
        };
      });
      const validMarks = subjectMarks.filter(sm => sm.obtained !== null && !sm.absent);
      const totalObtained = validMarks.reduce((s, m) => s + m.obtained, 0);
      const totalMax = validMarks.reduce((s, m) => s + m.max, 0);
      return {
        student,
        subjectMarks,
        totalObtained,
  ...
  const reportData = filteredMarks.map((m: any) => ({
    ...
    obtained: m.marks_obtained === -1 ? 0 : m.marks_obtained,
    max: m.max_marks,
  }));
                            ...
                            {row.subjectMarks.map((sm: any, i: number) => (
                              <TableCell key={i} className="text-center">
                                {sm.absent ? (
                                  ...
                                  <span className={sm.obtained / sm.max * 100 < 40 ? 'text-destructive font-medium' : ''}>
                                    {sm.obtained}/{sm.max}
                                  </span>
                                ) : (
